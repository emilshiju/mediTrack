...
const MedicationTable = () => {
  ...
  const [allMedicationDetails, setAllMedicationDetails] = useState<
    MedicationResType[]
  >([]);
  ...
  const fetchAllMedication = async () => {
    try {
      ...
      if (resAllMedicine.success == false) {
    ...
    if (window.confirm("Are you sure you want to delete this item")) {
      ...
      try {
        ...
        if (resdeleted.success && resdeleted.data.status) {
          const filteredData = allMedicationDetails.filter((a, b) => {
            if (a.id != id) {
              return a;
            }
          });
          setAllMedicationDetails(filteredData);
