...
export const createMedicationAssignApi = async (
  values: medicationAssignFormValuesType
) => {
  try {
    const resCreateMedicationAssignApi: resCustomType = await axiosClient.post(
      "/medication-assign",
      values
    );
...
export const findAssignedMedicationsApi = async (id: string) => {
  try {
    const resFindAssignedMedicationsApi: resCustomType = await axiosClient.get(
      `/medication-assign/patient/${id}`
    );
...
export const deleteAssignedMedicationApi = async (id: string) => {
  try {
    const resDeleteAssignedMedicationApi: resCustomType =
      await axiosClient.delete(`/medication-assign/${id}`);
